...
router.get("/:cat_id/subcategories", (req, res, next) => {
  try {
    const sql="select sub_cat_id,subcat_name from sub_categories where status=1 and cat_id='"+req.params.cat_id+"'";
    connection.query(
      sql,
      function(error, results, fields) {
...
router.get("/:subcat_id/servicetypes", (req, res, next) => {
  try {
    connection.query(
      "select ser_id,ser_name from service_types where status=1 and subcat_id='"+req.params.subcat_id+"'",
      function(error, results, fields) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      connection.query(
        "select id,username,email,fname,lname from users where email='" + email + "' and password='" + password + "'",
        function(error, results, fields) {
