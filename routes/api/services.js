...
router.get('/search/', [], async (req, res) => {
  ...
  if (req.query.category) {
    where += " and ss.cat_id='" + req.query.category + "' ";
  }
  ...
  if (req.query.seller_type) {
    ...
    req.query.seller_type.split('-').map((k) => {
      formatQuery.push("level_name='" + k + "'");
    });

    // where+= " and slv.name='"+req.query.seller_type+"' ";
    where1 += ' and (' + formatQuery.join(' or ') + ' )';
  }

  // if(req.query.seller_country){

  //   where+= " and ctry.name='"+req.query.seller_country+"' ";
  // }
  ...
  try {
    const sql =
      "select ss.id,ss.title,ss.price,ss.url_title,u.username,group_concat('/assets/uploads/users/',u.username,'/serviceImages/',si.image_path)  as images  from seller_services ss INNER JOIN business_categories bc ON bc.`cat_id`=ss.`cat_id` INNER JOIN users u ON ss.user_id = u.id   INNER JOIN countries ctry ON ctry.`id`=u.`country` INNER JOIN seller_levels slv ON  slv.`id`=u.`seller_level_id` INNER JOIN seller_languages selang ON selang.`user_id`=u.id INNER JOIN languages lang ON lang.`id`=selang.`lang_id` inner join services_images si on si.service_id=ss.id  " +
      where1 +
      ' group by ss.id;' +
...
router.post('/create-session-id', [auth], async (req, res) => {
  try {
    ...
    session.save((err, sn) => {
      if (sn) {
        res.json({ found: true, sessionID: sn._id });
      } else {
...
router.post('/get-session-data', [auth], async (req, res) => {
  try {
    ...
    connection.query(sql, function (error, results, fields) {
      ...
      if (results.length > 0) {
        res.json({ found: true, data: results[0][0] });
      } else {
        res.json({ found: false, data: null });
      }
    });
      ...
      if (results11.length > 0) {
        ...
        connection.query(sql, function (error, results, fields) {
          ...
          if (results.length > 0) {
            ...
            res.json({ found: true, data: [results1] });
          } else {
            res.json({ found: false, data: null });
          }
        });
