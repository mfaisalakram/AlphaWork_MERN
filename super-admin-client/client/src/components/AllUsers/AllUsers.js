...
const AllUsers = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/users/all-users').then((res) => {
  ...
  let ChangeHandler = (id, type) => {
    axios
      .put('http://localhost:5000/api/users/all-users', { id: id, type: type })
      .then((res) => {
  ...
  const ShowTypeHandler = (type) => {
    axios
      .post('http://localhost:5000/api/users/get-user-type', { type: type })
      .then((res) => {
        setusersData(res.data);
      });
  };
  ...
  const ShowNameHandler = (fname) => {
    axios
      .post('http://localhost:5000/api/users//get-user-byname', {
        fname: fname,
      })
      .then((res) => {
        setusersData(res.data);
