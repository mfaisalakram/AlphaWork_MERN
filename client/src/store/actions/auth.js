...
console.log(response.data);
      if(response.data.account_status=="banned"){
        ...
        dispatch({type: atypes.SET_TOAST,
          payload:[{msg:"your account is banned.please check  email for more details",type:"error"}]});
...
export const registerUser = ({fname,lname, email, password,username}) => async dispatch => {
  const config = {
    headers: {
      "Content-Type": "application/json"
    }
  ...
  await axios
    ...
    .then(response => {
      ...
      dispatch(setToast([{msg:"Register successfully.Redirecting to Login page",type:"success"}]));
      setTimeout(() => {
       window.location.href="/login"
      }, 1000);
    })
