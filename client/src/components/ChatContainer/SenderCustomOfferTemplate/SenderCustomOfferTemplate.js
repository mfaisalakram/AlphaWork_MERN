...
const SenderCustomOfferTemplate = ({ message, time, image, offer, offerId,buyer,sender}) => {
 const [status, setstatus] = useState(offer.status)
  ...
  const onwithdrawOffer=()=>{
    ...
    axios.post('/api/chatapp/offer-withdraw',data,config).then(res=>{
           setstatus("withdraw")
...
let btn;
  if(status=="send"){
    btn= <button className="btn btn-success pull-right" onClick={onwithdrawOffer}>Withdraw Offer</button>
